...
export default function EquipmentDetailClient({
  ...
  initialEquipment?: EquipmentDetail | null;
}) {
  const params = useParams();
  const router = useRouter();
  const equipmentId = params?.id as string;

  const [equipment, setEquipment] = useState<EquipmentDetail | null>(initialEquipment || null);
  const [isLoading, setIsLoading] = useState<boolean>(!initialEquipment);
  ...
  useEffect(() => {
    // If initialEquipment was provided by server, no need to re-fetch unless id changes
    if (initialEquipment && initialEquipment.id === equipmentId) {
