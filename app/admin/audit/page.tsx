          ...
          <div className="overflow-x-auto">
            <table className="w-full text-right text-xs text-slate-200 bg-slate-900">
              <thead className="bg-slate-950 text-slate-300 border-b border-slate-800 font-bold">
                <tr>
                  <th className="p-3.5 text-slate-300">نوع العملية</th>
                  <th className="p-3.5 text-slate-300">المستخدم / المشرف</th>
                  <th className="p-3.5 text-slate-300">الهدف / المورد</th>
                  <th className="p-3.5 text-slate-300">عنوان IP</th>
                  <th className="p-3.5 text-slate-300">الوقت والتاريخ</th>
                  <th className="p-3.5 text-slate-300">النتيجة</th>
                </tr>
              </thead>
              <tbody className="divide-y divide-slate-800/50">
                {[
                  ...
                  <tr key={idx} className="hover:bg-slate-800/50 transition font-mono">
                    ...
                    <td className="p-3.5 text-cyan-300 font-sans">{log.user}</td>
                    <td className="p-3.5 text-slate-300">{log.target}</td>
                    <td className="p-3.5 text-slate-300">{log.ip}</td>
                    <td className="p-3.5 text-slate-400 font-sans">{log.time}</td>
                    <td className="p-3.5">
                      <span className={`inline-flex items-center justify-center px-3 py-1.5 rounded-full text-[11px] font-sans font-bold whitespace-nowrap border ${log.color}`}>
                        {log.status}
                      </span>
                    </td>
                  </tr>
