  ...
  return (
    <header className="sticky top-0 z-50 w-full border-b border-gray-800/80 bg-gray-950/80 backdrop-blur-md">
      <div className="mx-auto flex h-18 max-w-7xl items-center justify-between px-4 sm:px-6 lg:px-8">
        ...
        <Link 
          href="/" 
          className="flex items-center gap-3 transition-opacity hover:opacity-95 focus:outline-none focus:ring-2 focus:ring-cyan-500 focus:ring-offset-2 focus:ring-offset-gray-950 rounded-lg"
          aria-label="Survsta Home"
        >
          <div className="relative h-10 w-36 sm:h-11 sm:w-40">
            <Image
              src="/images/Designer.png"
              alt="Survsta - Geomatics & Surveying SaaS"
              fill
              priority
        ...
        <nav className="hidden md:flex items-center gap-6 text-sm font-medium text-gray-300">
          <Link href="/providers" className="transition-colors hover:text-cyan-400">
            المكاتب والشركات
          </Link>
        ...
        <div className="hidden md:flex items-center gap-3">
          {user ? (
            <div className="flex items-center gap-3">
              ...
              <button
                onClick={() => { /* Placeholder logout action */ }}
      ...
      {/* Mobile Drawer */}
      {isMobileMenuOpen && (
        <div className="md:hidden border-b border-gray-800 bg-gray-950 px-4 py-4 space-y-3">
          <Link href="/providers" className="block py-2 text-sm text-gray-300 hover:text-cyan-400">
            المكاتب والشركات
          </Link>
